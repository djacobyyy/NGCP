...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
const float VH_MAX_HEADING = 200.0;
const float VH_MIN_HEADING = 120.0;

const float VH_MAX_VEL = 0.8;
const float VH_MIN_VEL = -0.8;
/* USER CODE END PM */
...
int main(void)
{
  ...
  vh_steering.travelAngle = 300.0;
  vh_steering.minLimit = 120.0; //120
  vh_steering.maxLimit = 200.0; // 200
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  duty_cycle = (int)(fabsf(0.9) * 480.0);
	  MotorControl_SetSpeed(&ugv_drive_mtr, 0.9, duty_cycle);
	*/
	  /*
	  if(vh_heading_direction){
		  vh_heading += 0.25;
	  	  if(vh_heading >= VH_MAX_HEADING)
	  		  vh_heading_direction = 0;
	  	  }
	  	  else{
	  		  vh_heading -= 0.25;
	  		  if(vh_heading <= VH_MIN_HEADING)
	  			  vh_heading_direction = 1;
	  }
	  ugv_servoSetAngle(&vh_steering, vh_heading);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	//udp_client_send();
	char log[200] = {0};
	sprintf(log, "Heading angle: %f, Velocity: %f, duty cycle: %d\r\n", vh_conv_heading, vh_vel_mag, duty_cycle);
